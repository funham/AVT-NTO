...
detector = Detector(
    classnames_path='/home/ubuntu/control/cv/obj.names',
    model_cfg='/home/ubuntu/control/cv/yolov4-tiny.cfg',
    model_weights='/home/ubuntu/control/cv/yolov4-tiny.weights',
)

first_building_point = [1.5, 3.8, 2.4]
second_building_point = [1.71, 1.82, 2.4]
...
def callback(event):  # функция обработки событй Автопилота
    ...
    if event == CallbackEvent.ENGINES_STARTED:  # блок обработки события запуска двигателя
        ...
        bboxes = []
        while len(bboxes) == 0:
            bboxes, _ = detector.forward(frame)
            print(bboxes[0].name)
            if name == [b.name for b in bboxes]:
                ...
                ap.goToLocalPoint(*second_building_point)
...
while not rospy.is_shutdown() and run:
    if board.runStatus() and not once:  # проверка подлкючения RPi к Пионеру
        ...
        cargo.on()
        pub = rospy.Publisher('/label', String)
