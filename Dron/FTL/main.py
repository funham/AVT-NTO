...
label_we_should_found = "peace"                                      # subtask3, subtask4
...
config_file = "./nn/conf.cfg"
thresh = 0.5
weights = "./nn/net.weights"
                    ...
                    if not should_center:
                        ...
                        if not should_countinue_after_finding_label_at_one_of_two_buildings:
                            ...
                            if not local_start:
                                ...
                                c = CountdownTask_test_only()
                                t = Thread(target = c.run, args = ())
                                t.start()

                            # fly_to_global_point(6.68, 1.52, altitude + 0.5)
                            # fly_to_local_point(-0.1, -0.1, altitude+0.5)
                            # fly_to_local_point(0.2, 0.2, altitude+0.5)
                            # fly_to_local_point(-0.1, -0.1, altitude+0.5)
                            rospy.sleep(5)
                            disarm()
                            c.terminate() 
...
#     fly_to_global_point(3.5, 4.5, 1.2)
# (3.5, 4.5, 1.2)
    # rospy.sleep(5)
# if local_start:
#     fly_to_global_point(3.5, 4.5, 1.2)
